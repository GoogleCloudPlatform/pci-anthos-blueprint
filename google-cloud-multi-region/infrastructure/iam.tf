...
resource "google_project_iam_member" "add_firewall_admin_to_app_gke_service_account" {
  project = google_project.network.project_id
  role    = "projects/${google_project.network.project_id}/roles/firewall_admin"
  member  = "serviceAccount:service-${google_project.app.number}@container-engine-robot.iam.gserviceaccount.com"
}
